...
public void OnPluginStart()
{
	...
	g_bRecordBots = CreateConVar("pstats_record_bots", "0", "Sets whether we should record bots", _, true, 0.0, true, 1.0);
	g_iStatsMenuTimeout = CreateConVar("pstats_menu_timeout", "30", "The timeout value for the player stats panel", _, true, 3.0, true, 9999.0);
	...
	g_bEnableExtraStats = CreateConVar("pstats_extras_enabled", "0", "Include extra stats to be recorded (Refer to the project documentation)", _, true, 0.0, true, 1.0);
	...
	g_bShowRankOnConnect = CreateConVar("pstats_show_rank_onjoin", "1", "If set, player rank will be displayed to the user on every map change", _, true, 0.0, true, 1.0);
	g_bConnectAnnounceEnabled = CreateConVar("pstats_cannounce_enabled", "1", "If set, connect announce will be displayed to chat when a player joins", _, true, 0.0, true, 1.0);
...
public void TQ_ShowPlayerRankPanel(Database db, DBResultSet results, const char[] error, DataPack pack) {
	...
	if (results == null) {
		...
		if (results.FetchRow()) {
			...
			PanelDrawStatItem(panel, "Survivor");
			
			PanelDrawStat(panel, "Kills", STATS_INFECTED_KILLED, map);
			PanelDrawStat(panel, "Headshots", STATS_INFECTED_HEADSHOT, map);
			
			PanelDrawStatLineBreak(panel);
			
			PanelDrawStatItem(panel, "Infected");
			
			PanelDrawStat(panel, "Kills", STATS_SURVIVOR_KILLED, map);
			PanelDrawStat(panel, "Incaps", STATS_SURVIVOR_INCAPPED, map);
...
public void TQ_ShowExtraStatsPanel(Database db, DBResultSet results, const char[] error, int client) {
	...
	if (results == null) {
		...
		if (results.FetchRow()) {
			...
			PanelDrawStat(panel, "Shotgun Skeets", STATS_EXTRA_SURV_SKEET_HUNTER_SHOTGUN, map);
			PanelDrawStat(panel, "Sniper Skeets", STATS_EXTRA_SURV_SKEET_HUNTER_SNIPER, map);
			PanelDrawStat(panel, "Melee Skeets", STATS_EXTRA_SURV_SKEET_HUNTER_MELEE, map);
			PanelDrawStat(panel, "Tank Rock Skeets", STATS_EXTRA_SURV_SKEET_TANK_ROCK, map);
			PanelDrawStat(panel, "Witch Crowns", STATS_EXTRA_SURV_WITCH_CROWN_STD, map);
			PanelDrawStat(panel, "Witch Draw Crowns", STATS_EXTRA_SURV_WITCH_CROWN_DRAW, map);
			PanelDrawStat(panel, "Boomer Pops", STATS_EXTRA_SURV_BOOMER_POP, map);
			PanelDrawStat(panel, "Charger Levels", STATS_EXTRA_SURV_CHARGER_LEVEL, map);
			PanelDrawStat(panel, "Tongue Cuts", STATS_EXTRA_SURV_SMOKER_TONGUE_CUT, map);
			PanelDrawStat(panel, "Deadstops", STATS_EXTRA_SURV_HUNTER_DEADSTOP, map);
			
			PanelDrawStatLineBreak(panel);
			
			PanelDrawStatItem(panel, "Infected");
			
			PanelDrawStat(panel, "Quad Booms", STATS_EXTRA_SI_BOOMER_QUAD, map);
			PanelDrawStat(panel, "25 Pounces", STATS_EXTRA_SI_HUNTER_25, map);
			PanelDrawStat(panel, "Death Charges", STATS_EXTRA_SI_DEATHCHARGE, map);
			PanelDrawStat(panel, "Tank Rock Hits", STATS_EXTRA_SI_TANK_ROCK_HITS, map);
